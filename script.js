...
async function getInput() {
    ...
    const meals = await fetchData(query);
    displayMeals(meals);
}
...
function addToFavourites(meal) {
    if (!myfavourites.some(fav => fav.idMeal === meal.idMeal)) {
        ...
        console.log(`${meal.strMeal} has been added to your favourites!`);
    }
}

function showFavourites() {
    const modalDiv = document.createElement('div');
    modalDiv.classList.add('favourites-modal')
    if (myfavourites.length === 0) {
        modalDiv.innerHTML = `<p>You have no favourite meals yet.</p>`;
    } else {
        modalDiv.innerHTML = `<h2>Your Favourite Meals</h2>`;
        myfavourites.forEach(meal => {
            modalDiv.innerHTML += `
                <div class="meal-card">
                    <img src="${meal.strMealThumb}" alt="${meal.strMeal}">
                    <h3>${meal.strMeal}</h3>
                    <button class="remove-favourite" data-id="${meal.idMeal}">Remove</button>
                    <button class="close-btn" data-id="${meal.idMeal}">Close button</button>
                </div>
            `;
        });
    }
